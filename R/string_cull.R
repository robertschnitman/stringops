...
#' 
#' @usage string_cull(s, pattern, collapse = ',', ...)
#' string_left(s, n)
#' string_right(s, n)
...
#' 
#' @details The function \code{string_cull()} culls (or extracts) a pattern from a string vector--if no pattern is found, \code{NA} is returned. Multiple pattern matches are separated by the \code{collapse()} input.
#' 
